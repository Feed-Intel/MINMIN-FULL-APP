...
export class RestaurantService {
  ...
  async listTenants(requestUserId: string): Promise<Tenant[]> {
    ...
    if (user.userType === 'restaurant' && user.managedTenant) {
      return this.tenantRepository.find({
        where: { id: user.managedTenant.id },
        relations: ['branches'],
      });
    }
  ...
  async getTenant(id: string, requestUserId: string): Promise<Tenant> {
    ...
    if (user.userType === 'restaurant' && user.managedTenant?.id === tenant.id) {
      return tenant;
    }
  ...
  private async loadUser(id: string): Promise<User> {
    const user = await this.usersRepository.findOne({
      where: { id },
      relations: ['managedTenant', 'branch', 'branch.tenant'],
    });
