...
export class AccountsService {
  ...
  async register(payload: CreateAccountDto): Promise<{ message: string }> {
    ...
    if (payload.branchId) {
      ...
      if (!branch.tenant) {
        throw new NotFoundException('Tenant not found for branch');
      }
    }
  ...
  async verifyOtp(payload: VerifyOtpDto): Promise<AuthTokens> {
    ...
    const user = await this.usersRepository.findOne({
      where: { email },
      relations: ['branch', 'branch.tenant', 'managedTenant'],
    });
  ...
  async login(payload: LoginDto): Promise<AuthTokens> {
    ...
    if (
      (user.userType === 'restaurant' && !user.managedTenant) ||
  ...
  private async issueTokens(user: User, includeRefresh = true): Promise<AuthTokens> {
    const payload = {
      ...
      tenant:
        user.managedTenant?.id ?? user.branch?.tenant?.id ?? null,
