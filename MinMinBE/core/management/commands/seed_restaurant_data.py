...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(email='admin@example3.com').exists():
            User.objects.create_superuser(
                email='admin@example3.com',
                password='password',
                full_name='Admin User',
            )
        demo_customer = User.objects.filter(email='customer@example3.com').first()
        if not demo_customer:
            demo_customer = User.objects.create_user(
                email='customer@example3.com',
                password='password',
        ...
        for r in range(restaurants):
            with transaction.atomic():
                admin = User.objects.create_user(
                    email=f'restaurant{r + 1}@example3.com',
                    password='password',
        ...
        for c in range(customer_count):
            customers.append(
                User.objects.create_user(
                    email=f'customer{c + 1}@example3.com',
                    password='password',
