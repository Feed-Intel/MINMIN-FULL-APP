...
__d(function (global, require, _$$_IMPORT_DEFAULT, _$$_IMPORT_ALL, module, exports, _dependencyMap) {
  ...
  async function GET(request) {
    if (!_constants.FACEBOOK_CLIENT_ID) {
    ...
    let platform;
    if (redirectUri === _constants.APP_SCHEME) {
      platform = "mobile";
    } else if (redirectUri === _constants.BASE_URL) {
      platform = "web";
    } else {
      return Response.json({
    ...
    const params = new URLSearchParams({
      ...
      redirect_uri: _constants.BASE_URL + "/api/auth/facebook/callback",
      response_type: "code",
      scope: scope,
      state: state
    });
    return Response.redirect(`${_constants.FACEBOOK_AUTH_URL}?${params.toString()}`);
  }
},3073,[1221]);
module.exports = __r(3073);