...
__d(function (global, require, _$$_IMPORT_DEFAULT, _$$_IMPORT_ALL, module, exports, _dependencyMap) {
  ...
  async function POST(request) {
    try {
      ...
      response.headers.set("Set-Cookie", `${_constants.COOKIE_NAME}=; Max-Age=0; Path=${_constants.COOKIE_OPTIONS.path}; ${_constants.COOKIE_OPTIONS.httpOnly ? "HttpOnly;" : ""} ${_constants.COOKIE_OPTIONS.secure ? "Secure;" : ""} SameSite=${_constants.COOKIE_OPTIONS.sameSite}`);

      // Clear the refresh token cookie
      response.headers.append("Set-Cookie", `${_constants.REFRESH_COOKIE_NAME}=; Max-Age=0; Path=${_constants.REFRESH_COOKIE_OPTIONS.path}; ${_constants.REFRESH_COOKIE_OPTIONS.httpOnly ? "HttpOnly;" : ""} ${_constants.REFRESH_COOKIE_OPTIONS.secure ? "Secure;" : ""} SameSite=${_constants.REFRESH_COOKIE_OPTIONS.sameSite}`);
      return response;
    } catch (error) {
